...
class ResistanceLandscape(nn.Module):
    ...
    - Heads:
        ...
        - State classification head: (n_states,) outputs → softmax
    ...
    def forward(self, fused_representation: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        Returns:
            Dict with keys:
                - "drug_resistance": (batch_size, n_drugs * n_timepoints) logits
                - "resistance_state": (batch_size, n_states) logits
        ...
        # Current resistance state (logits)
...
class ResistanceLandscapePredictor:
    ...
    def predict_single(
        ...
        Returns:
            LandscapeResult with all predictions.
        """
        # Add batch dimension
        ...
        state_logits = outputs["resistance_state"].squeeze(0)  # (n_states,)
        ...
        # Determine current state (argmax over softmax)
        state_probs = torch.softmax(state_logits, dim=0).cpu().numpy()
        ...
        # Confidence: mean probability of the predicted state
        confidence = float(state_probs[current_state_idx])
